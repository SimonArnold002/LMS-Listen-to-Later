...
> **STATUS: MERGED (2026-06) — historical.** This proposal was accepted as [PR #1235](https://github.com/CDrummond/lms-material/pull/1235), merged to `dev` (`b631754`) and then `master` (`519b03a`). The shipped Material exposes the service as `$SERVICE` and resolves the per-app `<command>-<type>` category with an `online-<type>` fallback. The plugin uses it via `svc:$SERVICE` (0.1.28). Kept for design context. **It IS in released Material: the feature shipped in 6.4.4** (tag `6.4.4`, master `9be80db`), so no bundle patching is needed — the "not released yet" note this line used to carry was true only in 2026-06 and was corrected 2026-09-10. Delivery has moved on again since: Material 6.4.6 added `registerCustomAction`, and the plugin registers rather than writing `actions.json` from 6.4.8. See CLAUDE.md, "How the Add entries REACH Material".
