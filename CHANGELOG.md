...
### Changed
- **A release moves to *Played* once you've heard 90% of it, rather than 60%.** The old figure was a hedge from when a release's length was half-guessed from what it was called; now that length is counted from the real track listing, there's nothing left to hedge against, and 60% of an album could move it to *Played* well before you'd finished it. The setting is still yours to change — this is only the starting point — but it's applied once to everyone on update, so you'll see the new figure even if you never touched it. Change it in Settings afterwards and it stays changed.
- **The 90% rounds down, so one skipped track never blocks a release.** Rounding up would have meant every single track of anything shorter than ten — 90% of nine tracks is 8.1, and rounded up that's all nine — so one track skipped, or one unavailable in your country, would have left the release stuck in your list forever. Rounded down, a nine-track album needs eight and a five-track one needs four. A two-track release is the one exception: it always needs both, because otherwise it would count as heard the moment it started.
- **The ♪ / ♫ / ❝ symbol has moved off the album title and onto the line below**, next to the word that describes the release and the service it came from — which is where it belongs, since it's telling you the same kind of thing. Titles now read as titles.

### Fixed
- **A release whose length couldn't be measured can now be measured later.** Listen Later asks a service how long a release is the first time you play it, and doesn't ask twice while it's waiting. If the service accepted that question and then never answered — no reply, no error — the wait never ended, so the release could never be asked about again for as long as the server stayed up. It sat on the fallback figure permanently, and anything shorter than that figure could never reach *Played* at all. The wait now expires after a minute, the next play asks again, and the lost request is noted in the server log instead of vanishing.

- **Albums saved from your own library, and from Qobuz, now get their release year.** Library albums added from a Material menu were arriving without one — Material has no year to hand there, even though it passes the album, so it's now read from your own library where it has always been known. Qobuz releases were missing it too whenever Qobuz states the date in its numeric form rather than as a written date; both are now read. This matters beyond the display: the year is part of how a duplicate is recognised, so an album saved without one wasn't spotted when the same album was later added from somewhere that supplies it, and you ended up with two rows.
- **Tidal, Deezer and Bandcamp releases added straight from those services still have no year**, and that's a limitation rather than an oversight: unlike Qobuz, those plugins don't offer album details to read a date from, only the track listing. The same releases added through ListenBrainz New Releases or Pitchfork Reviews do get a year, since those pass one over.

### Notes for people using the companion plugins
- **ListenBrainz New Releases and Pitchfork Reviews now pass over the album title the streaming service itself uses.** Both previously sent their own preferred name — MusicBrainz's for the first, Pitchfork's for the second — which reads better but is often not what the service calls the release. Since Listen Later recognises a release you're playing by its title, anything saved under a different name was never spotted and stayed in the list however often you played it. Update those plugins alongside this one. Releases already saved under the old names aren't repaired retrospectively; removing and re-adding one is enough.

## 0.1.92 — Releases the service names differently still reach Played

### Fixed
- **An album saved from ListenBrainz New Releases or Pitchfork Reviews now moves to *Played* even when the streaming service calls the release something slightly different.** Those plugins send the album's proper MusicBrainz name, which is the better one to save under — it's what makes a deluxe edition and a standard one count as the same record instead of two. But MusicBrainz doesn't always put a release's distinguishing part *in* the title: all four American Football albums are simply titled "American Football", with "LP2" and "LP3" recorded separately, while Qobuz and the others print "American Football (LP2)". Listen Later spots a release you're playing by its title, so a release saved under the shorter name was never recognised while it played — it stayed in the list forever, however often you heard it, and nothing looked wrong because it played perfectly. Listen Later now also remembers what the service itself called the release when it was added, and matches on either. Nothing to press, and what's shown in the list is unchanged.
- Releases already saved this way aren't repaired retrospectively — there's nothing stored to repair them from. Removing and re-adding one is enough. Albums added straight from a service's own pages were never affected.

## 0.1.91 — The row tells the truth about itself

### Changed
- **The ♪ / ♫ symbol on a row now follows what the release actually holds.** It says whether there's one track or several, so it's answered by counting — a release labelled *EP* that holds a single track shows ♪, and one labelled *Single* that holds three shows ♫. The wording beside it is left exactly as MusicBrainz or the service has it; only the symbol, which is ours, stops disagreeing with the track listing. Rows show the symbol from their label until they've been measured.

- **Qobuz releases added straight from a browse row now get their year.** Those rows don't carry one — the year only arrived when a release came from ListenBrainz New Releases, or from the Now Playing screen — so a good number of saved albums showed no year at all. Qobuz states the release date on the album details Listen Later already fetches when working out what a release is, so it's now read from there at no extra cost. A year that came with the add is never replaced, since it may be an original release date rather than a reissue's. This also stops those albums being saved twice: the year forms part of how duplicates are spotted, so an album saved without one wasn't recognised when the same album was later added from somewhere that supplies it. Bandcamp releases still have no year available unless they come via ListenBrainz New Releases.

### Fixed
- **A release whose title already ends with its year no longer shows it twice** — "Shearwater – The New World (2026) (2026)". Some feeds label their rows *Album (YYYY)* and Material passes that whole label over as the album name, so the year ends up inside the stored title and was then appended again. A title genuinely ending in a *different* year ("Live (1971)", released 2026) still gets its own year shown.

## 0.1.90 — Judging how long a release is, correctly

### Changed
- **How much of a release you must hear is now based on counting it, never on what it's called.** *Album*, *EP* and *Single* come from MusicBrainz or the streaming service, and they're frequently not literal — a release called a *Single* may hold three tracks, and one called an *EP* may hold exactly one. Listen Later used to read those labels as track counts, which went wrong in both directions: a three-track "single" moved to *Played* after one track, and a one-track "EP" could never move at all, because it was asked for two tracks it doesn't have. The labels still appear on the row, but a release's length is now always measured from its real track listing — looked up the first time you play it, if it isn't known already. Nothing to do; existing items re-measure themselves as you play them.

### Fixed
- **Streaming releases were being credited with tracks that aren't tracks.** When a release's track listing was fetched, extra rows the service includes alongside the music — *Artist*, *Credits*, *Music Label*, *Copyright*, *Add to favourites* — were counted as part of the release. A one-track release was recorded as six tracks and an eleven-track album as seventeen, so *Played* asked for far more of a release than it contains, and short releases could never get there at all. Only real, playable tracks are counted now, and every stored length is re-measured once.
- **A release added while its streaming service was briefly unreachable could still be marked as played too early.** Adding a release stores the type its source claims and then checks it a moment later against what the release actually holds. If that check couldn't reach the service it gave up silently, leaving a release described as a "single" — which this plugin takes to mean one track — so it moved to *Played* after a single track and, with auto-tidy on, was removed days later. The check is now retried, and if it still can't get an answer it says so in the server log rather than passing over it quietly. Opening or playing the release from your list has always corrected it, and still does.
- **A release with tracks unavailable in your country can reach *Played* again.** A release's length is now only ever taken from its actual track listing — the one your account can play — and never from a service's catalogue figure, which counts every track on the release whether it's available to you or not. A twelve-track album with five playable tracks was asking for eight before it counted as heard, so it never got there. The same applies to a release the service calls a "single" but lists as holding several tracks: that's now checked against the real track listing before it's treated as longer than one track, so a single whose extra tracks aren't available to you still moves to *Played* when you've heard it.
- **The follow-up length check can no longer stall without saying so.** If a service accepted the request and then simply never answered, the check waited forever — no retry, nothing in the log. It now gives up after a few seconds, says so, and takes its one retry.
- **A release whose track listing couldn't be fetched is no longer mistaken for a one-track release.** If the lookup failed, the "couldn't find this" placeholder was being counted as the release's one and only track — so an album could be marked *Played* after a single track, and a length it had already learned was overwritten with 1.
- **A release added before its type could be worked out no longer stays labelled "Album".** When the service was slow at add time the row went in unlabelled and showed the default; the answer arriving moments later was then thrown away. It's now filled in, without disturbing a type that's already known.
- **Adding a Bandcamp release no longer sets off a background search of the site.** The follow-up length check now only runs when the track listing can be fetched directly, which for Bandcamp needs the album page address rather than its id.

## 0.1.89 — Uses the track count from ListenBrainz New Releases when it's offered

### Changed
- **A release added from ListenBrainz New Releases can arrive already knowing how many tracks it holds**, if you're running that plugin at version 0.9.142 or later. It has looked the release up on the streaming service in order to find it, so it can pass the count along — which means a release that MusicBrainz calls a "single" but which really holds three tracks is recognised as an EP the instant the row appears, with nothing to look up. That count settles the *label* only; how much of a release you must hear before it moves to *Played* is always measured against its real track listing. Older versions of that plugin simply don't send it, and everything works exactly as it did in 0.1.88.

## 0.1.88 — Releases move to Played when you've actually heard them

### Fixed
- **A "single" that holds several tracks no longer jumps to *Played* after the first one.** Record labels and MusicBrainz both call a release a single when it has a lead track plus a B-side, a remix or a radio edit — but Listen Later treats *Single* as meaning one track, and marked the whole thing heard as soon as that one track finished. Any release described as a single is now checked against what it actually contains, and one with more tracks is treated as an EP (or an album, past six tracks) — so it needs a proper listen before it moves. Affects releases added from ListenBrainz New Releases and from Qobuz, whose own catalogue labels them the same way.
- **A short release could never reach *Played* at all.** With no track count to go on, a streaming release had to be played four tracks deep before it counted — which a release with fewer than four tracks can never manage, so it sat in the list forever however often you played it. One-track albums (which are a real thing, and correctly labelled) were the worst case. Releases now remember how many tracks they hold, so the same "most of it" rule as your library albums applies: a five-track album needs three, a three-track album needs two.
- Already-saved releases fix themselves. A release mislabelled as a single is corrected the first time you open or play it from the list, and picks up its track count at the same moment.

### Changed
- **Adding is still instant.** Working out what a release contains happens quietly in the background after it's been added, never while you wait — and on Qobuz it costs nothing at all, since the answer arrives with information the plugin already fetches.
